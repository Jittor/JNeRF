...
struct ConcentricSpheresIntersector {
    ...
    __device__ __host__ float _det(float r) {
        return f + 2 * q2a * r * r;
    }
...
__device__ __inline__ float trilerp_cuvol_one(
    ...
    const int idx) {
        // printf("offx offy stride idx %d %d %d %d\n",offx,offy,stride,idx);
...
#define MAYBE_READ_LINK(u) ((link_ptr[u] >= 0) ? data[link_ptr[u] * stride + idx] : 0.f)
    ...
    // printf("%f %f %f %f %f %f \n",ix0y0,ix0y1,ix0,ix1y0,ix1y1,ix1);
...
__device__ __inline__ void ray_find_bounds(
    ...
    // for (int i = 0; i < 3; i++) {
    //     printf("%f ", grid._scaling[i]);
    // }
    // for (int i = 0; i < 3; i++) {
    //     printf("%f ", grid._offset[i]);
    // }
    // printf("\n");
...
#define MAYBE_ADD_LINK_DEN(u, val)                   \
    if (link_ptr[u] >= 0) {                          \
...
#pragma unroll 3
    ...
    dir[0] = cam.info(CAM_NDCX) * (dir[0] / dir[2] - cen[0] / cen[2]);
    dir[1] = cam.info(CAM_NDCY) * (dir[1] / dir[2] - cen[1] / cen[2]);
    dir[2] = 2 * near / cen[2];

    cen[0] = cam.info(CAM_NDCX) * (cen[0] / cen[2]);
    cen[1] = cam.info(CAM_NDCY) * (cen[1] / cen[2]);
...
__device__ __inline__ void cam2world_ray(
    ...
    float x = (ix + 0.5f - cam.info(CAM_CX)) / cam.info(CAM_FX);
    float y = (iy + 0.5f - cam.info(CAM_CY)) / cam.info(CAM_FY);
