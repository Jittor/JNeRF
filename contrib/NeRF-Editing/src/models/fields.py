...
class SDFNetwork(nn.Module):
            ...
            if geometric_init:
                if l == self.num_layers - 2:
                    if not inside_outside:
                        jt.init.gauss_(lin.weight, mean=np.sqrt(np.pi) / np.sqrt(dims[l]), std=0.0001)
                        jt.init.constant_(lin.bias, -bias)
                    else:
                        jt.init.gauss_(lin.weight, mean=-np.sqrt(np.pi) / np.sqrt(dims[l]), std=0.0001)
                        jt.init.constant_(lin.bias, bias)
                    ...
                    jt.init.gauss_(lin.weight[:, :3], 0.0, np.sqrt(2) / np.sqrt(out_dim))
                elif multires > 0 and l in self.skip_in:
                    jt.init.constant_(lin.bias, 0.0)
                    jt.init.gauss_(lin.weight, 0.0, np.sqrt(2) / np.sqrt(out_dim))
    ...
    def gradient(self, x):
        y = self.sdf(x)
        gradients = jt.grad(y,x,retain_graph=True)  # same shape with x
        # gradients = jt.grad(y,x)  # same shape with x
        return gradients
...
class SingleVarianceNetwork(nn.Module):
    def __init__(self, init_val):
        ...
        # self.register_parameter('variance', nn.Parameter(jt.array(init_val)))
